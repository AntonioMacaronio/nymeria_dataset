...
def extract_to_mp4_chunked(sequence_folder: Path, output_dir: str, frame_rate: float = 30.0, resolution: int = 1408) -> List[Path]:
    ...
    The hdf5 file and mp4 file are then saved to the output directory.
    ...
    1. We look at the atomic_action.csv file to get a dataframe of all the atomic actions in the sequence.
        - In doing so, we also get the start_idx and end_idx of each atomic action.
    2. We extract the data for each atomic action by iterating through the frame timestamps and extracting the data for each frame.
    3. We save the video frames for each atomic action to a separate mp4 file.
    4. We save the other data for each atomic action to a separate hdf5 file with reference to the mp4.
    5. We return a list of the hdf5 file paths in sorted alphabetically order.
    ...
    for i in range(len(atomic_action_df)):
        ...
        with h5py.File(hdf5_path, 'w') as f:
            ...
            ########################################################
            ####  Step 3: Save video frames as MP4 file ######
            ########################################################
            # Create MP4 file for the egoview RGB frames
            mp4_path = Path(output_dir) / f"{sequence_name}_{i:05d}.mp4"

            if len(egoview_RGB_list) > 0:
                # Initialize VideoWriter (codec: mp4v for MP4 format)
                fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                video_writer = cv2.VideoWriter(str(mp4_path), fourcc, frame_rate, (resolution, resolution))

                # Write frames to video (convert from CHW to HWC and BGR)
                for rgb_frame in egoview_RGB_list:
                    # rgb_frame is (3, H, W) in RGB format
                    # Convert to (H, W, 3) in BGR format for OpenCV
                    frame_hwc = rgb_frame.transpose(1, 2, 0)  # CHW -> HWC
                    frame_bgr = cv2.cvtColor(frame_hwc.astype(np.uint8), cv2.COLOR_RGB2BGR)
                    video_writer.write(frame_bgr)

                video_writer.release()
                print(f"Saved MP4 file: {mp4_path} ({os.path.getsize(mp4_path) / 1e6:.2f} MB)")
            else:
                print(f"Warning: No frames to write for datapoint {i}")

            ########################################################
            ####  Step 4: Save other data as HDF5 file ######
            ########################################################
            # Store the MP4 filename in the HDF5 attributes
            f.attrs['egoview_mp4_filename'] = mp4_path.name
