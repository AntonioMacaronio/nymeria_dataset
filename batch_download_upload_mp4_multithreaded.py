...
def downloader_thread(
    ...
    for idx, key in enumerate(keys, start=1): # Ex: '20230607_s0_james_johnson_act0_e72nhq'
        ...
        if key in existing_keys:
            print(f"[Downloader] [{idx}/{total_keys}] ⏭️  Skipping (already in S3): {key}")
            continue
...
def uploader_thread(
    ...
    while True:
        ...
        print(f"[Uploader] [{idx}/{total_keys}] ☁️  Uploading: {key}")
        ...
        for hdf5_path in hdf5_file_paths:
            ...
            if not os.path.exists(str(hdf5_path).replace('.h5', '.mp4')):
                print(f"[Uploader] ⚠️  Skipping - HDF5 does not have a corresponding MP4: {hdf5_path}")
        ...
        for mp4_path in mp4_file_paths:
            ...
            if not os.path.exists(mp4_path):
                print(f"[Uploader] ⚠️  Skipping - MP4 does not exist: {mp4_path}")
                continue
        ...
        try:
            ...
            print(f"[Uploader] ⚠️  Cleanup warning for {key}: {e}")
...
def main(args: DownloadUploadArgs) -> None:
    ...
    └─────────────┘                    └─────────────┘                  └─────────────┘
         │                                   │                               │
         │ downloads raw                     │ extracts to                   │ uploads to S3
         │ sequence data                     │ hdf5 + mp4                    │ and cleans up
    ...
    if downloader.is_alive():
        print("⚠️  Warning: Downloader thread did not exit cleanly")
    if processor.is_alive():
        print("⚠️  Warning: Processor thread did not exit cleanly")
